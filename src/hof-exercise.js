...
const prices = (items) => items.map(
  item => item["price"]
); // TODO - Implementation

// sum: (numbers: [number]) -> number
const sum = (numbers) => numbers.reduce((sum, value) => sum + value, 0); // TODO - Implementation

// selectTaxable: (items: [{taxable: boolean}]) -> [{taxable: boolean}]
const selectTaxable = (items) => items.filter(
  item => item["taxable"]
); // TODO - Implementation

// applyTax: (prices: [number], tax: number) -> [number]
const applyTax = (prices, tax) => prices.map(
  item => item * tax
); // TODO - Implementation
